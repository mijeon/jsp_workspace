...
%>
<%
	//productDAO는 트랜잭션 때문에 SqlSession을 맴버변수로 두고, setter로 주입받길 기다리고 있으므로 (세션주입)
	SqlSession sqlSession=mybatisConfig.getSqlSession();
	productDAO.setSqlSession(sqlSession);
	
	//사용자가 넘긴 상위 카테고리를 이용해, 소속된 상품 가져오기
						...
						<div class="col-lg-4 col-md-6">
							...
							<%
								...
								if(product.getPimgList().size()>0){ 
									filename=product.getPimgList().get(i).getFilename();
								}else{
									out.print("파일없음");
								}
							%>
								<div class="product__item__pic set-bg"
									data-setbg="/data/<%=filename%>">
									<div class="label new">New</div>
									<ul class="product__hover">
										<li><a href="/data/<%=filename%>" class="image-popup"><span
